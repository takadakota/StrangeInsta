...
class InstaController extends Controller
{
    ...
    public function home(Request $request){
      ...
      $Tweet = Tweet::orderBy('tweet_id', 'desc')->simplePaginate(3);//ツイート全部を読み込んで降順に並び替えてる、仕様は１ページ１０ツイートだけどテスト用で３ツイート
              //simpleつけるとNextとPrevだけにできる（大量にあるときに有効）、こんな雑な１行でページ機能付与できるのすごい
    ...
    public function favoritesuser(Request $request){
      ...
      $favusers = Favorite::where('tweet_id',$tweet_id)->where('favorite',1)->get()->toArray();//tweet_idが一致している行を取得
    ...
    public function profile(Request $request){
      ...
      $count = Favorite::where('user_id',$user_id)->where('favorite',1)->count();
    ...
    */
    public function upload(Request $request)//画像とテキストが送られてくる。ここではそれらをテーブルに入れたい。
    {
      $request->validate([
        'file' => [
            // 必須
            'required',//kbyte、60MBが最大という制約入れたい
            // アップロードされたファイルであること
            'file',
            // 画像ファイルであること
            'image',
      ...
      $request->validate([
                'caption' => 'required|max:200',//Viewにもerror出力文書いたけど下のelse以下で処理してる
            ]);
      ...
      if ($request->file('file')->isValid([])) {//ファイルの中身があるなら
          ...
          $user = Auth::user();//メモ：Userテーブルから持ってくるとデータが汚い（コレクション型）、"カラム名:〜"　みたいになる
          $user_id = $user->user_id;
          $username = $user->username;
          $avatar = $user->avatar;
          $now = date("Y/m/d H:i:s");//ツイートしたときの日付時刻データ、降順にするのに使う
